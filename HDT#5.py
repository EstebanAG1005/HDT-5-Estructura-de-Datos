...
def simulacion (nombre, env, RAM, Procesadora,instruccion, Tprocesos, Tiempo, memoria,):
    ...
    while sim == True:
        ...
        while instruccion > 0:
            ...
            with Procesadora.request() as cola:
                ...
                if (Cprocesos == 1):
                  print("Se envia proceso %s a cola por %s segundos. Tiempo Actual: %s , Espacio en memoria: %s" % (nombre, Tiempo, env.now, memoria))
                  yield env.timeout(Tiempo)
                else:
                  print("Se envia el proceso %s a la memoria para ver su tiempo, Tiempo %s, Tiempo Actual: %s , Espacio en memoria: %s" % (nombre, Tprocesos, env.now, memoria))
                  RAM.put(memoria)
                  sim = False
